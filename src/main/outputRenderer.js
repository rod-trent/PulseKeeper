...
<body>
  ...
  <div class="layout">
    ...
    <main class="main">
      ...
      <div class="filter-bar" id="filterBar">
        <button class="filter-btn active" onclick="filterItems('all')">All (${items.length})</button>
        ${Object.entries(byType).map(([type, typeItems]) =>
          `<button class="filter-btn" onclick="filterItems('${type}')">${SOURCE_ICONS[type] || '📄'} ${type} (${typeItems.length})</button>`
        ).join('')}
      </div>
  ...
  <footer class="footer">
    Generated by Personal Content Builder · ${items.length} items from ${Object.keys(byType).length} source types
  </footer>
  <script>
    ...
    document.querySelectorAll('a.item-card').forEach(a => {
      a.addEventListener('click', e => {
        if (window.shell) { e.preventDefault(); window.shell.openExternal(a.href); }
      });
    });
  </script>
</body>
