...
async function init() {
  ...
  api.on.collectStart(() => setStatus('Collecting…', true));
  api.on.collectComplete(({ succeeded, failed }) => {
    setStatus(`Updated — ${succeeded} source${succeeded !== 1 ? 's' : ''} refreshed`, false);
    loadSources();
  });
...
function renderSourceCard(source) {
  ...
  return `<div class="source-card ${source.enabled ? '' : 'disabled'}" id="source-${source.id}">
    ...
    <div class="source-info">
      ...
      <div class="source-meta">${typeInfo.label || source.type} · Max ${source.maxItems || 20} items</div>
    </div>
...
async function saveSource() {
  ...
  try {
    if (editingSourceId) {
      await api.sources.update(editingSourceId, { name, type, config, maxItems });
      showToast('Source updated');
    } else {
      await api.sources.add({ name, type, config, maxItems });
...
function getSourceIcon(type) {
  const m = { rss:'📡', podcast:'🎙️', youtube:'▶️', twitter:'🐦', spotify:'🎵', reddit:'🔴', newsletter:'📧', blog:'✍️', webpage:'🌐', 'web-capture':'🧩' };
  return m[type] || '📄';
}
